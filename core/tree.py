...
class Tree (object):
    ...
    technicalDeps=set(("ellipsis","omistus","gsubj","name"))
    ...
    def buildPOSTags(self):
        ...
        try:
            ...
            resTW=py_fincg.analyzeSentence(txt,False,True) #TWOL, tokenization
            resCG=py_fincg.analyzeSentence(txt,True,True) #CG, tokenization
        except RuntimeError:
            return
        ...
        for dep in self.deps.values():
            ...
            for tag in dep.flags:
                if tag.startswith(u"ARG"):
                    ...
                    outLines.append(u"flag(%d,%d,%d,%d,'%s','%s')."%(plTsetIdx,plId,dep.gov,dep.dep,dep.type,PLEsc(tag)))
...
#             return #UNIMPLEMENTED
    ...
    def save_conllu(self,out,sent_id=None):
        ...
        for idx,t in enumerate(self.tokens):
            ...
            tags=t.posTags[0][2].split(u"|",1)
            cols[idx][3]=tags[0].replace(u"POS_",u"")
            cols[idx][4]=cols[idx][3]
            if len(tags)>1:
                assert len(tags)==2
                cols[idx][5]=tags[1]
            cols[idx][6]=u"0"
        ...
        for idx,dheads in enumerate(heads):
            ...
            if self.tokens[idx].misc!=u"_":
                misc=[self.tokens[idx].misc]
            else:
                misc=[]
            misc+=[dh.misc for dh in dheads if dh.misc and dh.misc!=u"_"]

            if self.layer(dheads[0])==1:
                ...
                cols[idx][6]=u"0"
                cols[idx][7]=u"root"
            #print >> sys.stderr, misc
            if misc:
                cols[idx][9]=(u"|".join(misc)).replace(u" ",u"_")
            else:
                cols[idx][9]=u"_"
            ...
            for d in dheads:
                if self.layer(d)==1:
                    print >> sys.stderr, str(sent_id),"layer collision", d.type, d.flags, "with", cols[idx][7]
            cols[idx][8]=u"|".join(unicode(d.gov+1)+u":"+d.type for d in dheads)
        for c in self.comments:
            if not c.startswith(u"# visual-style"):
                print >> out, c.encode("utf-8")
        for (g,d,t),dep in self.deps.iteritems():
            if u"converted" in dep.flags and u"L2" in dep.flags:
                print >> out, "# visual-style %d %d %s\tcolor:brown"%(g+1,d+1,t)
            elif u"converted" in dep.flags:
                print >> out, "# visual-style %d %d %s\tcolor:red"%(g+1,d+1,t)
            elif u"L2" in dep.flags:
                print >> out, "# visual-style %d %d %s\tcolor:blue"%(g+1,d+1,t)
